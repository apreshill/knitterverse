...
knitted_summary <- knitted_data %>% 
  ...
  # grab even ages only
  filter(age %% 2 == 0) %>% 
  select(age, word, prop_produce, kids_respond)
```

## Explore ages

```{r ages}
ggplot(knitted_summary, aes(x = age, y = kids_respond)) +
...
```{r}
write_csv(knitted_data, 
          here::here("data","knitted_data.csv"))
write_csv(knitted_summary, 
          here::here("data","knitted_summary.csv"))
```
